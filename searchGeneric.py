# search() methods modified to store additional results
...
class AStarSearcher(Searcher):
    ...
    def __init__(self, problem):
        self.goalpaths = []
        self.endflag = False # set to true when reach goal
    ...
    def add_to_frontier(self,path):
        ...
        # overriding parent class' search() method
    def search(self):
        """returns (next) path from the problem's start node
        to a goal node. 
        Returns None if no path exists.
        """
        if self.endflag is False: # so the following is not checked again if already finished search
            while not self.empty_frontier():
                path = self.frontier.pop()
                self.display(2, "Expanding:",path,"(cost:",path.cost,")")
                self.num_expanded += 1
                if self.problem.is_goal(path.end()):    # solution found
                    self.display(1, self.num_expanded, "paths have been expanded and",
                                len(self.frontier), "paths remain in the frontier")
                    self.solution = path   # store the solution found
                    self.goalpaths.append(path.end()) #store only the goal node
                    return path
                else:
                    neighs = self.problem.neighbors(path.end())
                    self.display(3,"Neighbors are", neighs)
